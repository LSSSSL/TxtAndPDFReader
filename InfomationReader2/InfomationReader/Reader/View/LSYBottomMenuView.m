...
-(UIButton *)catalog
{
    if (!_catalog) {
        ...
        [_catalog setImage:[UIImage imageWithContentsOfFile:[LSYReadUtilites getImagWithPath:@"reader_cover@2x.png"]] forState:UIControlStateNormal];
        _catalog.hidden = YES;
    }
    return _catalog;
}
...
-(void)layoutSubviews
{
    ...
    _increaseFont.frame = CGRectMake(DistanceFromLeftGuiden(_fontLabel), DistanceFromTopGuiden(_slider)+10,  (ViewSize(self).width-20)/3, 30);
     _catalog.frame = CGRectMake(10,  DistanceFromTopGuiden(_increaseFont)+10, 40, 40);
    _themeView.frame = CGRectMake(50, DistanceFromTopGuiden(_increaseFont)+10, ViewSize(self).width-100, 40);
